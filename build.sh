...
export CROSS_COMPILE="$HOME/android/tc/uber/out/arm-eabi-5.x-cortex-a15/bin/arm-eabi-"
KERNEL_DIRECTORY="$HOME/android/kernel/Velvet-N4"
ANYKERNEL_DIRECTORY="$HOME/android/kernel/anykernel_msm"
...
if [[ "$1" =~ "cm" || "$1" =~ "CM" ]] ; then
kernelzip="Velvet-V$VERSION-CM-$DEVICE.zip"
...
kernelzip="Velvet-V$VERSION-$DEVICE.zip"
fi
...
# Clean the /zip directory if there have been a zip file before (Mostly a version of Velvet compiled.)
if [ -e zip/*.zip ] ; then
rm -rf zip/*
fi
